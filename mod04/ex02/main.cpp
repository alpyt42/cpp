...
/*   Updated: 2023/05/08 19:59:20 by ale-cont         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main ( void )
{
	...
	std::cout << std::endl << " --- Test abstracts class--- " << std::endl;
	// Animal	animal; --> impossible, we need to provide a pointer
	Animal	*animal = NULL;
	animal = &cat; // if we don't change the type, it will create a segfault when calling makesound;
	std::cout << std::endl << animal->getType() << " | " << animal->getType() << std::endl;
	animal->makeSound();
	std::cout << std::endl;
	return (0);
}